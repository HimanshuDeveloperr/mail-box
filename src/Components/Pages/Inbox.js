...
const Inbox = () => {
  ...
  const readHandler = async (id) => {
    ...
    try {
      await axios.patch(`https://mail-box--inbox-default-rtdb.firebaseio.com/${sentMail}/${id}.json`, {
        classes: true
      });
    } catch (error) {
      console.log(error);
            ...
            <>
              {sentboxArr.length === 0 ? (
                ...
                sentboxArr.map((email) => (
                  <div key={email.id} className="mb-3 pb-3 border-bottom">
                    <p className="mt-2">
                      {email.classes ? <span className="text-info">--</span> : <span className="text-danger">--</span>}
                      <strong>mail from:</strong> {email.from}
                    </p>
                    <p>
                      <strong>Subject:</strong> {email.subject}
                    ...
                    <Link to={`/inboxmessages/${email.id}`} className="text-decoration-none">
                      <Button variant="outline-info" onClick={() => readHandler(email.id)}>
                        View Message
                      </Button>
                    </Link>
                    <Button variant="outline-danger" className="mx-2" onClick={() => deleteHandler(email.id)}>
